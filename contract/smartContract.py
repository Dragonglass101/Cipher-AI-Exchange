...
class Contract(sp.Contract):
    def __init__(self):
        self.init( 
            user_keys = sp.map(tkey=sp.TAddress, tvalue=sp.TRecord(public_key=sp.TString, private_key=sp.TString)),
            creators = sp.map(tkey=sp.TAddress, tvalue=sp.TRecord(model_name=sp.TString, link_encoding=sp.TString, price=sp.TMutez)),
            rentals = sp.map(tkey=sp.TAddress, tvalue=sp.TString)
        )
        
    
    @sp.entry_point
    def set_user_key(self, params):
        # Set the user's public and private keys
        self.data.user_keys[params.user_address] = sp.record(public_key=params.public_key, private_key=params.private_key)

    @sp.entry_point
    def add_creator(self, params):
        self.data.creators[params.user_address] = sp.record(
            model_name = params.model_name,
            link_encoding = params.link_encoding,
            price = params.price
        )

    @sp.entry_point
    def remove_creator(self, params):
        # sp.update_map(self.data.creators, params.address, sp.none)
        del self.data.creators[params.user_address]

    @sp.entry_point
    def buy_model(self, params):
        sp.verify(sp.amount == self.data.creators[params.owner_address].price, "Invalid Amount")
        sp.send(params.owner_address, sp.amount, "Transaction Failed") 
        

    @sp.entry_point
    def add_rental(self, params):
        self.data.rentals[params.user_address] = params.link_encoding

    @sp.entry_point
    def remove_rental(self, params):
        # sp.update_map(self.data.rentals, params.user_address, sp.none)
        del self.data.rentals[params.user_address]
    
if "templates" not in __name__:
    @sp.add_test(name="Contract tests")
    def test():
        scenario = sp.test_scenario()

        #test_accounts
        nunnu = sp.test_account("Nunnu")
        rakshit = sp.test_account("Rakshit")
        rahul = sp.test_account("Rahul")

        contract = Contract()
        scenario.h1("Contract Tests")
        scenario += contract

        #add new user, private and public keys in map
        scenario += contract.set_user_key(user_address = rahul.address, public_key = "20", private_key = "21")
        scenario += contract.set_user_key(user_address = rakshit.address, public_key = "10", private_key = "13")
        scenario += contract.set_user_key(user_address = nunnu.address, public_key = "30", private_key = "33")

        #check user
        scenario.p("getNunnu").show(contract.data.user_keys[nunnu.address])
        scenario.p("getRakshit").show(contract.data.user_keys[rakshit.address])
        scenario.p("getRahul").show(contract.data.user_keys[rahul.address])

        #add creator model
        scenario += contract.add_creator(user_address = nunnu.address, model_name="Mode1l", link_encoding = "modelnunnu", price=sp.tez(100))

        #buy model
        scenario += contract.buy_model(owner_address = nunnu.address).run(sender = rakshit, amount=sp.tez(100))
        
        #add rentals
        scenario += contract.add_rental(user_address = rakshit.address, link_encoding = "rakshit_encrypt_nunnu")
        scenario += contract.add_rental(user_address = rahul.address, link_encoding = "rahul_encrypt_nunnu")

        #remove rental
        scenario += contract.remove_rental(user_address = rahul.address)

        

        
    sp.add_compilation_target("contract", Contract())
